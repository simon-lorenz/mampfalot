...
const request = supertest('http://localhost:5001/authenticate')

describe('Authentication', () => {
	describe('/authenticate', () => {
		describe('GET', () => {
			it('authenticates a user with correct credentials', async () => {
				await request.get('/').auth('maxmustermann', '123456').expect(200)
			})

			it('authenticates a user with umlauts in his username', async () => {
				await request.get('/').auth('björn_tietgen', 'test').expect(200)
			})

			it('authenticates a user with owasp special chars in his password', async () => {
				await request.get('/').auth('luisa-rogers', ' !"#$%&\'()*+,-./:;<=>?@[\\]^_`{|}~').expect(200)
			})
			...
			it('fails with 401 if auth header is missing', async () => {
				await request.get('/').expect(401).expect(Boom.unauthorized('Missing authentication').output.payload)
			})
