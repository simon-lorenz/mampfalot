...
router.route('/').get((req, res, next) => {
	...
	const credentialsB64 = basicAuth.split(' ')[1]
	const credentials = Buffer.from(credentialsB64, 'base64').toString('utf-8') // Enthält nun username:password
	...
	const password = splitted.slice(1, splitted.length).join(':')
...
router.route('/').get(asyncMiddleware(async (req, res, next) => {
	...
	if (user) {
		...
		const passwordMatch = await bcrypt.compare(res.locals.credentials.password, user.password)
		if (!passwordMatch) {
