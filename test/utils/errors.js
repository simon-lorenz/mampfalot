module.exports = {
	...
	checkValidationError(error, item) {
		...
		if (item && !this.validationErrorItemExists(error, item)) {
			throw new Error(
				`Searched validationErrorItem ${JSON.stringify(item)} was not found in ${JSON.stringify(error.errors)}`
			)
		}
	},
	...
	validationErrorItemExists(error, searchedItem) {
		for (const item of error.errors) {
			if (
				item.field === searchedItem.field &&
				item.value === searchedItem.value &&
				item.message === searchedItem.message
			) {
				return true
			}
		}
	...
	checkRequiredBodyValues(error, values, all) {
		if (all) {
			this.checkRequestError(
				error,
				`This request has to provide all of the following body values: ${values.join(', ')}`
			)
		} else {
			this.checkRequestError(
				error,
				`This request has to provide at least one of the following body values: ${values.join(', ')}`
			)
		}
	},
	...
	checkRequiredQueryValues(error, values, all) {
		if (all) {
			this.checkRequestError(
				error,
				`This request has to provide all of the following query values: ${values.join(', ')}`
			)
		} else {
			this.checkRequestError(
				error,
				`This request has to provide at least one of the following query values: ${values.join(', ')}`
			)
		}
	}
}
